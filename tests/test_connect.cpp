...
TEST_CASE("connections with curve authentication", "[curve][connect]") {
    ...
    KreditsMQ client{get_logger("C» "), LogLevel::trace};
...
TEST_CASE("plain-text connections", "[plaintext][connect]") {
    std::string listen = "tcp://127.0.0.1:4455";
    KreditsMQ server{get_logger("S» "), LogLevel::trace};
...
TEST_CASE("unique connection IDs", "[connect][id]") {
    ...
    KreditsMQ client1{get_logger("C1» "), LogLevel::trace};
    KreditsMQ client2{get_logger("C2» "), LogLevel::trace};
    client1.start();
    client2.start();
...
TEST_CASE("SN disconnections", "[connect][disconnect]") {
    std::vector<std::unique_ptr<KreditsMQ>> lmq;
    ...
    for (int i = 0; i < pubkey.size(); i++) {
        lmq.push_back(std::make_unique<KreditsMQ>(
...
TEST_CASE("SN single worker test", "[connect][worker]") {
    ...
    KreditsMQ client{get_logger("B» "), LogLevel::trace};
    client.start();
