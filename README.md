# KreditsMQ - zeromq-based message passing for Kredits projects

This C++14 library contains an abstraction layer around ZeroMQ to support integration with Kredits
...
and local daemon commands, communication channels between kreditsd and auxiliary services (storage
server, kreditsnet), and also provides a local multithreaded job scheduling within a process.
...
KreditsMQ messages come in two fundamental forms: "commands", consisting of a command named and
...
either direction at any time.  KreditsMQ sets up a "listener" and "client" connections, but these only
...
`category.command` - for commands/requests registered by the KreditsMQ caller (e.g. kreditsd).  Here
...
on this below).  For example, for kreditsd categories are:
...
the request via KreditsMQ you provide a callback to be invoked when the reply arrives.  On the wire
this looks like:
...
by KreditsMQ (both the invoker and the recipient code only see the `hello world`/`dlrow olleh` message
parts).
...
Optional command/request arguments are always strings on the wire.  The KreditsMQ-using developer is
...
Internal commands passed between KreditsMQ-managed threads use either plain strings or bt-encoded
dictionaries.  See `kreditsmq/bt_serialize.h` if you want a bt serializer/deserializer.
...
- a string containing a service node pubkey.  In this mode KreditsMQ will look for the given SN in
  already-established connections, reusing a connection if one exists.  If no connection already
  exists, a new connection to the given SN is attempted (this requires constructing the KreditsMQ
...
The callbacks are passed a KreditsMQ::Message object from which the message (plus various connection
...
connection to deliver the message.  In order for this to work the KreditsMQ caller must provide a
lookup function to retrieve the remote address given a SN x25519 pubkey.

### Callbacks

Invoked command functions are always invoked with exactly one arguments: a non-const KreditsMQ::Message
reference from which the connection info, KreditsMQ object, and message data can be obtained.
...
also allows configuration of how public connections should be treated: for example, a kreditsd running
...
For example, in kreditsd the categories described above have authentication levels of:
...
In order to handle ServiceNode authentication, KreditsMQ uses an Allow callback invoked during
...
KreditsMQ operates a pool of worker threads to handle jobs.  The simplest use just allocates new jobs
...
As mentioned above, KreditsMQ tries to avoid exceeding the configured general threads value (G)
...
Instead KreditsMQ provides a mechanism for this by allowing you to submit a batch of jobs with a
...
void continue_big_task(std::vector<kreditsmq::job_result<double>> results) {
    double sum = 0;
    for (auto& r : results) {
        try {
            sum += r.get();
...
void start_big_task() {
    ...
    kreditsmq::Batch<double /*return type*/> batch;
...
exhausting the worker threads with waiting jobs) then you can implement it yourself; KreditsMQ isn't
...
KreditsMQ supports scheduling periodic tasks via the `add_timer()` function.  These timers have an
