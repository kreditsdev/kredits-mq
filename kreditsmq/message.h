...
class Message {
public:
    KreditsMQ& kreditsmq; ///< The owning KreditsMQ object
    ...
    /// Constructor
    Message(KreditsMQ& lmq, ConnectionID cid, Access access, std::string remote)
        : kreditsmq{lmq}, conn{std::move(cid)}, access{std::move(access)}, remote{std::move(remote)} {}
